...
- *Client load generation*: Clients interact via adaptors to drive a benchmark load, determined by a rate control mechanism.
...
![Benchmark](../_static/benchmark.png)
...
- Reporting stage: Statistics from all clients of each test round are analyzed, and an HTML format report will be generated automatically.

    The default directory path for the generated report is the workspace directory, and the file is named `report.html`. You can override this setting in the following ways:
    ...
    - Using a configuration file to override the `caliper.report.path` property.

!!! note

    It is the user’s responsibility to ensure that the directory hierarchy exists between the workspace and the report file.

A report example is as below:

![Report](../_static/report.png)
